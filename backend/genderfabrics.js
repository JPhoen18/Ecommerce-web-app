  ...
  .then(async (res) => {
    ...
    fabricsArray = res.rows;
        for(var obj of fabricsArray){
          ...
          console.log('this.entiremainmenspage');
          console.log(entiremainmenspage);
          //return;
          if(entiremainmenspage){

            await pool.query('SELECT * FROM itemsecondimage WHERE itemsecondimageid=$1',[obj.clothesid])
            .then((res) => {
                 console.log("Here is the item info!");
                 console.log(res);
             ...
             await pool.query('SELECT * FROM itemthirdimage WHERE itemthirdimageid=$1',[obj.clothesid])
             .then((res) => {
                console.log("Here is the item info!");
                console.log(res);
          ...
          .then((res) => {
               ...
               console.log(res);
               imgurl = res.rows;
               fabricsinfo = {'fabricsinfo':obj, 'iteminfo':iteminfo, 'itemimage':imgurl}
               if(checksecondthird){
               fabricsinfo = {'fabricsinfo':obj, 'iteminfo':iteminfo, 'itemimage':imgurl, 'itemsecondimage':seconditemimage, 'itemthirdimage':thirditemimage}
             }
...
async function getGenderFabrics(gend, pol,  mainmenspage, callback){
 gender = gend;
 pool = pol;
 var finalobj;
...
console.log(mainmenspage);

entiremainmenspage = mainmenspage;
console.log('gender fabrics mens main page');
console.log(entiremainmenspage);
